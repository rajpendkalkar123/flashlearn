...
class _AiFlashcardCreationScreenState extends State<AiFlashcardCreationScreen> {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['candidates'] != null && data['candidates'].isNotEmpty) {
        ...
        if (content != null && content['parts'] != null && content['parts'].isNotEmpty) {
          ...
          // Log the extracted flashcards and quizzes
          print("Extracted Flashcards: $flashcards");
          print("Extracted Quizzes: $quizzes");
          ...
          if (flashcards.isNotEmpty || quizzes.isNotEmpty) {
            ...
            DocumentReference setRef = await FirebaseFirestore.instance.collection('flashcardSets').add({
              'userId': uid,
              'setTitle': promptText, // Use the input text as the set title
            ...
            // Save each generated quiz in the 'quizzes' collection
            if (quizzes.isNotEmpty) {
              for (var quiz in quizzes) {
                await FirebaseFirestore.instance.collection('quizzes').add({
                  'quizTitle': promptText, // Quiz title
                  'description': 'AI-generated quiz',
                  'userId': uid,
                  'questions': quiz['questions'], // Store questions
                  'correctAnswers': quiz['correctAnswers'], // Store correct answers
                });
              }
            }
  ...
  // Function to extract flashcards from the generated text
  List<Map<String, String>> _extractFlashcards(String generatedText) {
    List<Map<String, String>> flashcards = [];
    var lines = generatedText.split('\n');
    for (var line in lines) {
      if (line.isNotEmpty) {
        var parts = line.split(':'); // Assuming "Term: Definition" format
        if (parts.length == 2) {
          flashcards.add({
            'term': parts[0].trim(),
  ...
  // Function to extract quizzes from the generated text
  List<Map<String, dynamic>> _extractQuizzes(String generatedText) {
    ...
    for (var line in lines) {
      if (line.startsWith('Q:')) {
        questions.add(line.replaceFirst('Q:', '').trim());
      } else if (line.startsWith('A:')) {
        correctAnswers.add(line.replaceFirst('A:', '').trim());
      }
    }

    // Log the extracted questions and correct answers
    print("Questions: $questions");
    print("Correct Answers: $correctAnswers");

    // Only add quizzes if there are both questions and answers
    if (questions.isNotEmpty && correctAnswers.isNotEmpty) {
      quizzes.add({
        'questions': questions,
        'correctAnswers': correctAnswers, // Store correct answers
      });
    }
